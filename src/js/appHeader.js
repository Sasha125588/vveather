...
import { handleVVeatherByGeolocation } from "./geolocation.js";
import { cToF, fToC, resetVVeatherContent } from "./helper.js";
...
export const createHeader = (city) => {
  ...
  searchBtn.addEventListener("click", async () => {
    ...
    try {
      const vveather = await getData(searchInput.value);

      if (vveather.message) {
        showError(vveather.message);
        return;
      }
      resetVVeatherContent(vveather.name, vveather);
    } catch (err) {
      console.log(err);
    }
  });
  ...
  cityLocation.addEventListener("click", handleVVeatherByGeolocation);
